...
install_apt () {
  ...
  DEBIAN_FRONTEND=noninteractive `#no prompting` \
    ...
    keepassxc xdotool chrome-gnome-shell xclip pavucontrol `#tools` \
    vim vim-scripts vim-runtime vim-doc curl xd libguestfs-tools ecryptfs-utils `#systools` \
    lm-sensors p7zip-full libimage-exiftool-perl screen baobab `#systools` \
    ...
    pidgin `#chatapps` \
    ansible `#automation` \
    google-chrome-stable insync keybase `#extra repos` \
    docker.io
}
...
print_text2art 'Install Java 11'
