...
alias docker-clear-hass='HASS_CONTAINER=$(docker inspect homeassistant | jq -r '.[].Id');  sudo sh -c "> /var/lib/docker/containers/$HASS_CONTAINER/$HASS_CONTAINER-json.log"'
...
alias nginx-reopn="/usr/bin/docker exec nginx /bin/sh -c '/usr/sbin/nginx -s reopen'"
